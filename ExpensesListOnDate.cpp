...
// заносим остаток на конец дня в БД
...
{
//    MessageDlg("Не верно выбрана дата !!! Нет исходных данных для перерасчета !!!",mtWarning,TMsgDlgButtons() << mbOK,0);
    Date1Edit->Date=Date();
//    Date1Edit->SetFocus();
    return;
}else
...
{
// здесь делаем перерасчет по дням
...
{
    MessageDlg("Перерасчет не нужен !!! ",mtWarning,TMsgDlgButtons() << mbOK,0);
    return;
}
*/
}
