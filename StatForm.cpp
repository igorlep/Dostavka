...
MessageDlg("Неверно указаны даты начала и конца периода. Будьте внимательны!!! ",mtWarning,TMsgDlgButtons() << mbOK,0);
return;
}
...
int index = autoNumberEdit->ItemIndex;
    ...
    if (GSM_DM->UserRole!="администратор" && GSM_DM->UserRole!="руководитель") sql+=" and expenses_table.u_id="+IntToStr(GSM_DM->UserID);
...
sql="";
    ...
    if (GSM_DM->UserRole!="администратор" && GSM_DM->UserRole!="руководитель" && GSM_DM->UserRole!="бухгалтер") sql+=" and expenses_table.u_id="+IntToStr(GSM_DM->UserID);
    sql+=" and oper_table.visible = 1";
